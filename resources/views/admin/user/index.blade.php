            ...
            <div class="kt-portlet">
                <div class="kt-portlet__head">
                    ...
                    <div style="text-align: right; padding-top: 0.5%;">
                        <a href="javascript:void(0)" value="{{ route('admin.user.create') }}" class="btn btn-success modalShow" title="Tambah Data" data-toggle="modal" data-target="#modalShow"><i class="fa fa-plus-circle"></i> Tambah Pengguna</a>
                    </div>
                </div>
                        ...
                        <table class="table table-striped table-hover table-bordered" id="myDataTables">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">{{ $item->roles->first()->name }}</td>
                                    <td class="align-middle">{{ \App\Services\QueryService::getInstance()->getInstitutionByUserId($item->id)->name }}</td>
                                    <td class="align-middle text-center">
                                    	<span class="btn btn-xs btn-{{ ($item->is_active) ? 'success' : 'danger' }}">{{ isActive($item->is_active) }}</span>
                                    </td>
                                    <td class="align-middle text-center">
                                        <a href="javascript:void(0)" value="{{ route('admin.user.password.change', ['id' => $item->id]) }}" class="btn btn-sm btn-icon btn-warning modalShow" data-toggle="modal" title="Ubah Password" data-target="#modalShow"><i class="fa fa-lock"></i></a>
                                        <a href="javascript:void(0)" value="{{ route('admin.user.edit', ['id' => $item->id]) }}" class="btn btn-sm btn-icon btn-info modalShow" data-toggle="modal" title="Ubah Data" data-target="#modalShow"><i class="fa fa-edit"></i></a>
                                        <form style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')" action="{{ route('admin.user.delete', ['id' => $item->id]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
