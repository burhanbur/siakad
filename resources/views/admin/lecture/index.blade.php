            ...
            <div class="kt-portlet">
                ...
                <div class="kt-portlet__head">
                    ...
                    <div style="text-align: right; padding-top: 0.5%;">
                        <a href="javascript:void(0)" value="{{ route('admin.lecture.create') }}" class="btn btn-success modalShow" title="Tambah Data" data-toggle="modal" data-target="#modalShow"><i class="fa fa-plus-circle"></i> Tambah Dosen</a>
                    </div>
                </div>
                        ...
                        <table class="table table-striped table-hover table-bordered" id="myDataTables">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle text-center">{{ @$item->institution->getAttributes()['code'] }}</td>
                                    <td class="align-middle text-center">{{ $item->name }}</td>
                                    <td class="align-middle text-center">{{ $item->email }}</td>
                                    <td class="align-middle text-center">{{ $item->phone }}</td>
                                    <td class="align-middle text-center">{{ @\App\Services\QueryService::getInstance()->getInstitutionById($item->institution->getAttributes()['institution_id'])->name }}</td>
                                    <td class="align-middle text-center">{{ isActive($item->is_active) }}</td>
                                    <td class="align-middle text-center">
                                        <a href="javascript:void(0)" value="{{ route('admin.lecture.edit', ['id' => $item->id]) }}" class="btn btn-sm btn-icon btn-info modalShow" data-toggle="modal" title="Ubah Data" data-target="#modalShow"><i class="fa fa-edit"></i></a>
                                        <form style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')" action="{{ route('admin.lecture.delete', ['id' => $item->id]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
